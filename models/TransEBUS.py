...
class TransEBUS(nn.Module):
    def __init__(self,
        ...
        self.tokenizer = Tokenizer(n_input_channels=n_input_channels,
                                   ...
                                   conv_bias=False)
        UD_sequence_length = self.tokenizer.sequence_length(n_channels=n_input_channels,
                                                            height=img_height,
                                                            width=img_width,
                                                            depth=img_depth)
        if self.two_stream:
            self.tokenizer_E = Tokenizer_E(n_input_channels=n_input_channels,
                                           ...
                                           n_conv_layers=n_conv_layers,
                                           conv_bias=False)
            E_sequence_length = self.tokenizer_E.sequence_length(n_channels=n_input_channels,
                                                                 height=img_height,
                                                                 width=img_width)
        else:
            E_sequence_length = 0
        ...
        self.classifier = TransformerClassifier(
            sequence_length=UD_sequence_length + E_sequence_length,
