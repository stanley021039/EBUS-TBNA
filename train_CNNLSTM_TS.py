...
import json
from models.CNN_LSTM_TS import CNNLSTMUDE
...
parser.add_argument("--save_dir", default='savemodel220628_CNNLSTM_TS', type=str, help="assign save folder")
...
def test(epoch, dataloader, Net, loss_fn):
    ...
    optim_threshold_v, optim_cf_v, auc_v = Roc_curve(np.array(y_true_video), np.array(y_pred_video),
                                                     savedir=save_dir + '/Roc_curve/test_v_' + str(epoch) + '.jpg', re_auc=True)
    print('Per lesion')
    plot_confusion_matrix_and_scores(hist_lesion, savedir=save_dir + '/confusion_matrix/test_l_' + str(epoch) + '.jpg')
    optim_threshold_l, optim_cf_l, auc_l = Roc_curve(np.array(y_true_lesion), np.array(y_pred_lesion),
                                                     savedir=save_dir + '/Roc_curve/test_l_' + str(epoch) + '.jpg', re_auc=True)
    return loss / total_v, Accuracy_v, Accuracy_l, optim_threshold_v, optim_threshold_l, auc_v, auc_l
...
model = nn.DataParallel(CNNLSTMUDE(num_classes=number_classes, init_weights=True), device_ids=[0, 1])
model.to(device)
...
if is_training:
    TrainingDataset = Dataset3D('data/raw_video', 'data_lesion_0524.xlsx', 'data/two_stream_data_old/UD_clip', doppler=True, elastography=True,
                                ...
                                resize=(224, 224), UD_shape='THWC', E_shape='HWC')
    ValidationDataset = Dataset3D('data_lesion_0524', 'data_lesion_0524.xlsx', 'data/two_stream_data_old/UD_clip', doppler=True, elastography=True,
                                  ...
                                  UD_reconstruct=False, E_reconstruct=False, resize=(224, 224), UD_shape='THWC', E_shape='HWC')
...
# testing
if is_testing:
    TestingDataset = Dataset3D('data/raw_video', 'data_lesion_0524.xlsx', 'data/two_stream_data_old/UD_clip', doppler=True, elastography=True,
                               E_dir='data/two_stream_data_old/E_img', sample_rate=sample_rate,
