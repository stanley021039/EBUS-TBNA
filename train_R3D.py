...
import json
from models.R3D_Modify import R3D_UD
...
parser.add_argument("--save_dir", default='savemodel220602_R3D', type=str, help="assign save folder")
...
def valid(epoch, dataloader, Net, loss_fn):
    ...
    optim_threshold_v, optim_cf_v, auc_v = Roc_curve(np.array(y_true_video), np.array(y_pred_video),
                                                     savedir=save_dir + '/Roc_curve/valid_v_' + str(epoch) + '.jpg', re_auc=True)
    print('Per lesion')
    ...
    optim_threshold_l, optim_cf_l, auc_l = Roc_curve(np.array(y_true_lesion), np.array(y_pred_lesion),
                                                     savedir=save_dir + '/Roc_curve/valid_l_' + str(epoch) + '.jpg', re_auc=True)
...
model = nn.DataParallel(R3D_UD(num_classes=number_classes, time_steps=time_steps, init_weights=True), device_ids=[0, 1])
model.to(device)
...
if is_training:
    TrainingDataset = Dataset3D('data/raw_video', 'data_lesion_0524.xlsx', 'data/two_stream_data_old/UD_clip',
                                doppler=True, elastography=False, E_dir='data/two_stream_data_old/E_img',
                                sample_rate=sample_rate,
                                time_steps=time_steps, number_classes=number_classes, dataset_split=['train'],
                                augmentation=aug_list, UD_reconstruct=False, E_reconstruct=False, UD_shape='THWC')
    ValidationDataset = Dataset3D('data/raw_video', 'data_lesion_0524.xlsx', 'data/two_stream_data_old/UD_clip',
                                  doppler=True, elastography=False, E_dir='data/two_stream_data_old/E_img',
                                  sample_rate=sample_rate,
                                  time_steps=time_steps, number_classes=number_classes, dataset_split=['valid'],
                                  UD_reconstruct=False, E_reconstruct=False, UD_shape='THWC')
    ...
    for epoch in range(1, Epoch):
        ...
        v_loss, v_Acc_video, v_Acc_lesion, optim_th_video, optim_th_lesion, aucv, aucl = valid(epoch, ValidationLoader, model, loss_fn)
...
#testing
if is_testing:
    TestingDataset = Dataset3D('data/raw_video', 'data_lesion_0524.xlsx', 'data/two_stream_data_old/UD_clip', doppler=True, elastography=False,
    ...
    test_loss, test_Acc_video, test_Acc_lesion, th_v, th_l, auc_v, auc_l = test(epoch, TestingLoader, model, loss_fn, threshold, threshold)
