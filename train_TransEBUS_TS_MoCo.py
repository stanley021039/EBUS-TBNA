...
import json
from models.TransEBUS_v1 import CCT_UDE
...
parser.add_argument("--save_dir", default='savemodel220726_TransEBUS_TS_MoCo', type=str, help="assign save folder")
...
def train_MOCO(epoch, dataloader, encoder, momentum_encoder, queue_0, queue_1, optimizer, loss_fn, temperature=1):
    ...
    count_1 = 0
    hist = np.zeros((2, 2))
    for i, (data_q, data_k, target, _, _) in enumerate(tqdm(dataloader, ncols=80)):
        # Preprocess
        encoder.train()
        momentum_encoder.train()
        encoder.zero_grad()
        x_q, e_q = data_q[0].to(device), data_q[1].to(device)
        ...
        if config.shuffle_bn:
            ...
            x_k = x_k[idx]
        '''
        pred_q, q = encoder(x_q, e_q)  # q : (N, 128)
        pred_k, k = momentum_encoder(x_k, e_k)  # k : (N, 128)
...
def valid(epoch, dataloader, encoder, loss_fn):
    ...
    y_pred_lesion = []
    with torch.no_grad():
        for i, (data, e_img, target, name, stime) in enumerate(tqdm(dataloader, ncols=80)):
            data, target, e_img = data.to(device), target.to(device), e_img.to(device)
            pred, q = encoder(data, e_img)
...
if is_training:
    ...
    f.close()
    TrainingDataset = Dataset3D_MOCO('data_lesion_0524', 'data_lesion_0728.xlsx', 'data/two_stream_data_old/UD_clip',
                                     doppler=True, elastography=True,
                                     E_dir='data/two_stream_data_old/E_img', resize=(224, 224), sample_rate=sample_rate,
    ...
    ValidationDataset = Dataset3D_MOCO('data_lesion_0524', 'data_lesion_0728.xlsx', 'data/two_stream_data_old/UD_clip',
                                       doppler=True, elastography=True,
                                       E_dir='data/two_stream_data_old/E_img', resize=(224, 224), sample_rate=sample_rate,
                                       time_steps=time_steps, number_classes=number_classes, dataset_split=['valid'],
                                       UD_reconstruct=False, E_reconstruct=False, E_top=3)
    ...
    optimizer = optim.SGD(encoder.parameters(), lr_, momentum=0.9, weight_decay=5e-4)
    ...
    queue_1 = []
    with torch.no_grad():
        for i, (data_q, data_k, target, _, _) in enumerate(TrainingLoader):
            _, key_feature = momentum_encoder(data_k[0].to(device), data_k[1].to(device))
    ...
    for epoch in range(1, Epoch + 1):
        ...
        plot_fig(np.array(lr_list), savedir=save_dir + "/lr.jpg")
        plot_fig(np.array(train_loss_list), savedir=save_dir + "/train_loss.jpg")
        plot_fig(np.array(train_loss_neg_list), savedir=save_dir + "/train_loss_neg.jpg")
        plot_fig(np.array(train_loss_pos_list), savedir=save_dir + "/train_loss_pos.jpg")
        plot_fig(np.array(train_Acc_list), savedir=save_dir + "/train_acc.jpg")
        plot_fig(np.array(valid_loss_list), savedir=save_dir + "/valid_loss.jpg")
        ...
        if valid_auc_lesion > max_auc:
            ...
            print('train loss: %.4f' % train_loss, 'loss_neg: %.4f' % train_loss_0, 'loss_pos: %.4f' % train_loss_1, file=f)
            print('valid loss: %.4f' % valid_loss, 'valid Acc: %.2f' % valid_Acc_lesion, 'valid AUC: %.4f' % valid_auc_lesion, file=f)
...
if is_testing:
    f = open(save_dir + "/epoch_log.txt", 'a')
    TestingDataset = Dataset3D_MOCO('data_lesion_0524', 'data_lesion_0728.xlsx', 'data/two_stream_data_old/UD_clip',
                                    doppler=True, elastography=True,
                                    E_dir='data/two_stream_data_old/E_img', resize=(224, 224), sample_rate=sample_rate,
                                    time_steps=time_steps, number_classes=number_classes, dataset_split=['test'],
                                    UD_reconstruct=False, E_reconstruct=False, E_top=3)
    TestingLoader = DataLoader(TestingDataset, batch_size=test_batch, shuffle=False, num_workers=8)

    model = nn.DataParallel(encoder, device_ids=[0, 1])
    model.to(device)
    epoch = 193
