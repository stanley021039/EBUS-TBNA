...
parser.add_argument("--save_dir", default='savemodel220921_TransEBUS_TS', type=str, help="assign save folder")
...
# model = CCT(img_size=(224, 224, 12), embedding_dim=192, n_input_channels=3, n_conv_layers=1, )
...
#flops, params = profile(model, ((inp_x, inp_e),))
...
if is_training:
    ...
    # pretrain
    pre_train_epoch = 168
    pretrain_path = 'savemodel220712_CCT_3UD_SimCLR/model_state_dict/CCT_Pretrain_' + str(pre_train_epoch)
    # pretrain_path = 'savemodel220712_CCT_3UD_SimCLR/model_state_dict/ResUDE_156'
    state_dict = torch.load(pretrain_path)
    keys = []
    for k, v in state_dict.items():
        if 'positional_emb' in k or 'fc' in k:
            print('del:', k)
            continue
        print(k)
        keys.append(k)
