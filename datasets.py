...
import time
from utils import E_build, UD_npy_bulid, UD_2Dimg_bulid, label_smoothing, two_stream_dataset_builder
...
class Dataset3D(Dataset):  # numpy type data
    def __init__(self, video_dir, excel_dir, UD_dir, E_dir=None, doppler=False, elastography=False, sample_rate=0.25,
                 time_steps=12, number_classes=1, dataset_split=['train'], augmentation=None, dataset_build=False,
                 crop_size='r1', E_crop_size='Er1', resize=None, E_resize=None, UD_shape='CHWT',
        ...
        if dataset_build:
            exc = pd.read_excel(excel_dir)
            two_stream_dataset_builder(exc, video_dir, UD_dir, E_dir, (748, 1452, 160, 736), time_steps, sample_rate, size=crop_size, resize=resize)

        for folder in os.listdir(E_dir):  # all elasography image
            for LN in os.listdir(os.path.join(E_dir, folder)):
                E_top_list = []
                self.E_filedict[LN] = []
                for img in os.listdir(os.path.join(E_dir, folder, LN)):
                    E_img_path = os.path.join(self.E_dir, folder, LN, img)
                    if E_top != 0 and len(E_top_list) >= E_top:
                        E_img = cv2.imread(E_img_path)
                        color_pixel = np.sum(np.max(E_img, axis=2) - np.min(E_img, axis=2) > 30)
                        if color_pixel > min(E_top_list):
                            index = E_top_list.index(min(E_top_list))
                            self.E_filedict[LN][index] = E_img_path
                            E_top_list[index] = color_pixel
                        else:
                            continue
                    elif E_top != 0:
                        E_img = cv2.imread(E_img_path)
                        color_pixel = np.sum(np.max(E_img, axis=2) - np.min(E_img, axis=2) > 30)
                        self.E_filedict[LN].append(E_img_path)
                        E_top_list.append(color_pixel)
                    else:
                        self.E_filedict[LN].append(E_img_path)
        ...
        for (i, LN) in self.LNs_exc.iterrows():  # construct input list
            ...
            for clip in os.listdir(ln_npy_dir):
                ...
                num_clip = clip.replace('.npy', '').split('_')[1]
                graph = np.load(os.path.join(ln_npy_dir, clip.replace('clip', 'graph')))
                for E_path in self.E_filedict[video_folder]:
                    self.filename.append((video_folder, clip_path, E_path, graph, num_clip, cls))
        ...
        print('-Benign slices:', B_count_data)
        print('-Malignant slices:', M_count_data)

    def __getitem__(self, index):
        lesion_name, clip_npy_path, E_path, graph, num_clip, tumor_class = self.filename[index]
        ...
        if self.isE:
            ...
            return video, e_image, graph, label, lesion_name, num_clip
            ...
            UD_transpose = (
                ...
                UD_transpose_dict[self.UD_shape[3]])
            video = torch.from_numpy(np.transpose(video.copy(), UD_transpose)).float()
            return video, graph, label, lesion_name, num_clip
...
class Dataset3D_MoCo(Dataset):  # numpy type data
            ...
            for clip in os.listdir(ln_npy_dir):
                ...
                if self.isE:
                    if dataset_split == ['train']:
                        self.filename.append((video_folder, clip_path, self.E_filedict[video_folder], graph, num_clip, cls))
                        if cls == "benign" or cls == "Benign":  # data count
                            B_count_data += 1
                        else:
                            M_count_data += 1
                    ...
                    self.filename.append((video_folder, clip_path, [''], graph, num_clip, cls))
                    if cls == "benign" or cls == "Benign":  # data count
                        B_count_data += 1
                    else:
                        M_count_data += 1
    ...
    def __getitem__(self, index):
        lesion_name, clip_npy_path, E_list, graph, num_clip, tumor_class = self.filename[index]
        _, _, _, folder, LN, clip_name = clip_npy_path.split('/')
        aug_dict = {'GaussianBlur': HorizontalFlip, 'AddGaussianNoise': GaussianNoise,
        ...
        if self.isE:
            ...
            e_q = e_image
            e_k = e_image
            if self.augmentation is not None:  # train
                for (aug, probability) in self.augmentation:  # same aug for UD and E
                    if random.random() < probability:
                ...
                video_q = torch.from_numpy(np.transpose(video_q.copy(), UD_transpose)).float()  # transpose to input shape
                ...
                return (video_q, e_q), (video_k, e_k), graph, label, lesion_name, num_clip
            else:  # valid or test
            ...
            if self.augmentation is not None:
                ...
                return video_q, video_k, graph, label, lesion_name, num_clip
            else:
                video = torch.from_numpy(np.transpose(video, UD_transpose)).float()
                return video, graph, label, lesion_name, num_clip
