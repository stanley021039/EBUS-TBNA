...
parser.add_argument("--save_dir", default='savemodel220709_ViT_3D', type=str, help="assign save folder")
...
model = ViT(image_size=(224, 224, 12), patch_size=(28, 28, 1), num_classes=1, dim=512, depth=6, heads=12, mlp_dim=1024, dropout=0.1)
...
if is_training:
    TrainingDataset = Dataset3D('data/raw_video', 'data_lesion_0524.xlsx', 'data/two_stream_data_old/UD_clip', doppler=True, elastography=False,
                                ...
                                augmentation=aug_list, UD_reconstruct=False, E_reconstruct=False, UD_shape='CHWT')
    ValidationDataset = Dataset3D('data/raw_video', 'data_lesion_0524.xlsx', 'data/two_stream_data_old/UD_clip', doppler=True, elastography=False,
                                  E_dir='data/two_stream_data_old/E_img', resize=(224, 224), sample_rate=sample_rate,
                                  time_steps=time_steps, number_classes=number_classes, dataset_split=['valid'],
                                  UD_reconstruct=False, E_reconstruct=False, UD_shape='CHWT')
    TrainingLoader = DataLoader(TrainingDataset, batch_size=train_batch, shuffle=True, num_workers=8, pin_memory=True)
    ValidationLoader = DataLoader(ValidationDataset, batch_size=valid_batch, shuffle=False, num_workers=8, pin_memory=True)
...
if is_testing:
    TestingDataset = Dataset3D('data/raw_video', 'data_lesion_0524.xlsx', 'data/two_stream_data_old/UD_clip', doppler=True, elastography=False,
                               ...
                               UD_reconstruct=False, E_reconstruct=False, resize=(224, 224), UD_shape='CHWT')
