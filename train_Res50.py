...
parser.add_argument("--save_dir", default='savemodel220621_Res50', type=str, help="assign save folder")
...
fc_fe = model.fc.in_features
model.fc = nn.Linear(fc_fe, 1)
model = nn.DataParallel(model, device_ids=[0, 1])
model.to(device)
...
if is_training:
    ...
    for epoch in range(1, Epoch):
        ...
        if aucl > max_auc:
            max_auc = aucl
            best_epoch = epoch
            torch.save(model.state_dict(), save_dir + "/model_state_dict/Res50_" + str(epoch))
...
if is_testing:
    ...
    epoch = best_epoch
    model.load_state_dict(torch.load(save_dir + "/model_state_dict/Res50_" + str(epoch)))
