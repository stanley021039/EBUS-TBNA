...
class R3D_UDE(nn.Module):
    def __init__(self, num_classes, init_weights=False, time_steps=24, Elayertofc=False, mode=True, f_cat=False, o_act=True):
        super(R3D_UDE, self).__init__()
        self.mode = mode
        self.f_cat = f_cat
        self.o_act = o_act
        self.Elayertofc = Elayertofc
        self.conv3D = nn.Conv3d(3, 16, kernel_size=5, padding=0, bias=False)
        ...
        self.avg_pool = nn.AvgPool3d(kernel_size=(3, 3, time_steps-4), stride=2)
        if not f_cat:
